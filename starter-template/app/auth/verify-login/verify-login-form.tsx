...
import { resendCodeAction, verifyLoginAction } from "@/actions/auth";
...
export default function VerifyLoginForm({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await verifyLoginAction(phoneNumber, code, newPassword);
      if (result.redirect) {
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                <Button type="submit" className="w-full" disabled={isPending}>
                  {isPending ? "در حال تأیید..." : "تأیید و تغییر رمز"}
                </Button>
              </form>
            </CardContent>
          </Card>
